...
def paraphrase_command(text, slots, tokenizer, model, device, num_variations=3, num_beams=5):
    ...
    input_ids = tokenizer.encode(text, return_tensors="pt", max_length=128, truncation=True).to(
        device
    )
...
if __name__ == "__main__":
    ...
    if latest_input_file:
        ...
        original_dataset = []
        with open(latest_input_file) as f:
            for line in f:
        ...
        for entry in tqdm(original_dataset):
            ...
            try:
                new_texts = paraphrase_command(
                    entry["text"], entry["slots"], tokenizer, model, device
                )
        ...
        print(
            f"Error: No base dataset file found in '{INPUT_DIR}'. Please run 'generate_slot_dataset.py' first."
        )
