...
if __name__ == "__main__":
    ...
    if not latest_pegasus_file and not latest_flan_t5_file:
        ...
        print(
            f"Found Pegasus file: {os.path.basename(latest_pegasus_file) if latest_pegasus_file else 'None'}"
        )
        print(
            f"Found FLAN-T5 file: {os.path.basename(latest_flan_t5_file) if latest_flan_t5_file else 'None'}"
        )
