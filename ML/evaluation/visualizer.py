...
class MetricsVisualizer:
    ...
    def plot_per_intent_performance(self, version, metrics, top_n=10):
        ...
        # Sort by F1 score
        sorted_intents = sorted(per_intent.items(), key=lambda x: x[1]["f1"], reverse=True)[:top_n]
        ...
        ax.set_title(
            f"Top {top_n} Intent Performance - Model v{version}", fontsize=14, fontweight="bold"
        )
        ax.set_xticks(x)
